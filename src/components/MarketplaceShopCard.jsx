...
export default function MarketplaceShopCard({ shop, dist, onOpen, onWhatsApp }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', width: '100%', aspectRatio: '16 / 9', background: photo ? '#0F172A' : 'linear-gradient(135deg,#4F46E5,#6366F1)', overflow: 'hidden' }}>
        {photo ? (
          ...
          <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <img src={logo} alt={shop.name} loading="lazy" onError={(e) => { e.currentTarget.style.display = 'none'; }}
              style={{ width: 64, height: 64, borderRadius: 14, objectFit: 'cover', background: '#fff', border: '2px solid rgba(255,255,255,0.7)', boxShadow: '0 4px 12px rgba(0,0,0,0.18)' }} />
          </div>
        ) : (
          <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 40, fontWeight: 800 }}>{initial}</div>
        )}
        ...
        {/* status pill */}
        <span style={{ position: 'absolute', top: 8, left: 8, fontSize: 9.5, fontWeight: 700, padding: '2px 7px', borderRadius: 999, background: open ? '#10B981' : '#64748B', color: '#fff', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
          <span style={{ width: 4, height: 4, borderRadius: '50%', background: '#fff' }} />{open ? 'Open' : 'Closed'}
        </span>
        {shop.subscription && shop.subscription !== 'trial' && (
          <span style={{ position: 'absolute', top: 8, right: 8, fontSize: 8.5, fontWeight: 800, padding: '2px 6px', borderRadius: 5, background: '#F59E0B', color: '#fff' }}>PRO</span>
        )}
        {/* distance chip */}
        <span style={{ position: 'absolute', bottom: 7, left: 8, fontSize: 10.5, fontWeight: 600, color: '#fff', display: 'inline-flex', alignItems: 'center', gap: 2 }}>
      ...
      <div style={{ padding: '10px 11px', display: 'flex', flexDirection: 'column', gap: 5, flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#64748B', textTransform: 'capitalize', flexWrap: 'wrap' }}>
          ...
          {rating ? (
            ...
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2, fontWeight: 600, color: '#4F46E5' }}>
              <Star size={11} color="#4F46E5" />New
            </span>
          )}
          {eta && (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2, color: '#64748B' }}>
              <Clock size={11} />~{eta}m
            </span>
          )}
        </div>
        <div style={{ display: 'flex', gap: 7, marginTop: 'auto', paddingTop: 6 }}>
          <button onClick={(e) => { e.stopPropagation(); onOpen?.(); }}
            style={{ flex: 1, padding: '8px', background: '#4F46E5', color: '#fff', border: 'none', borderRadius: 9, fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}>
            Order now
          </button>

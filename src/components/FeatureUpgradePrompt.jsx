  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <Sparkles size={20} color="var(--c-surface)" />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--c-ink)', letterSpacing: '-0.01em' }}>
          {title || `Upgrade to ${requiredPlan}`}
        </div>
        <div style={{ fontSize: 12, color: 'var(--c-ink-2)', marginTop: 4, lineHeight: 1.4 }}>
          {body || 'This feature is available on higher plans.'}
        </div>
        {hint && (
          <div style={{ fontSize: 11, color: 'var(--c-muted)', marginTop: 6, fontStyle: 'italic' }}>{hint}</div>
        )}
      </div>
      <button

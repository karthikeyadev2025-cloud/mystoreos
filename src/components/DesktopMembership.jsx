...
const EMPTY_PLAN = {
  ...
  discount_percent: 10, free_services: 0, color: '#8B5CF6', active: true,
};
...
function PlanCard({ plan, onEdit, onDelete, onIssue }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 14, padding: 20, display: 'flex', flexDirection: 'column', gap: 12, borderTop: `4px solid ${plan.color}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 800, fontSize: 16, color: '#0F172A' }}>{plan.name}</div>
          {plan.description && <div style={{ fontSize: 12, color: '#64748B', marginTop: 2 }}>{plan.description}</div>}
        </div>
        <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
          <button onClick={() => onEdit(plan)} title="Edit"
            style={{ width: 28, height: 28, borderRadius: 6, border: '1px solid #E2E8F0', background: '#F8FAFC', color: '#4F46E5', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Edit2 size={13} /></button>
          <button onClick={() => onDelete(plan.id)} title="Delete"
            style={{ width: 28, height: 28, borderRadius: 6, border: '1px solid #FEE2E2', background: '#FFF5F5', color: '#EF4444', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Trash2 size={13} /></button>
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
        <span style={{ fontSize: 26, fontWeight: 900, color: plan.color }}>₹{Number(plan.price).toLocaleString('en-IN')}</span>
        <span style={{ fontSize: 12, color: '#94A3B8' }}>/ {plan.duration_days} days</span>
      </div>
      ...
      <button onClick={() => onIssue(plan)}
        style={{ marginTop: 4, padding: '8px 12px', borderRadius: 8, border: 'none', background: plan.color, color: '#fff', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
        + Issue to Customer
      </button>
    </div>
  );
  ...
  return (
    <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 12, padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 4 }}>Duration</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 6 }}>Card Colour</label>
        <div style={{ display: 'flex', gap: 6 }}>
          {COLORS.map(c => (
            <button key={c} onClick={() => setForm(p => ({ ...p, color: c }))}
              style={{ width: 30, height: 30, borderRadius: 8, border: form.color === c ? '3px solid #0F172A' : '1px solid #E2E8F0', background: c, cursor: 'pointer' }} />
          ))}
        </div>
      </div>
...
function IssueMembershipForm({ plan, shopId, onSaved, onCancel }) {
  ...
  return (
    <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 12, padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div>
        <div style={{ fontWeight: 800, fontSize: 15, color: '#0F172A' }}>Issue {plan.name}</div>
        <div style={{ fontSize: 12, color: '#64748B', marginTop: 2 }}>₹{Number(plan.price).toLocaleString('en-IN')} · {plan.duration_days} days · {plan.discount_percent}% off</div>
      </div>
      <div>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 4 }}>
        <button onClick={onCancel} style={{ padding: '8px 18px', borderRadius: 8, border: '1px solid #E2E8F0', background: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
        <button onClick={save} disabled={saving}
          style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: plan.color, color: '#fff', fontSize: 13, fontWeight: 700, cursor: saving ? 'wait' : 'pointer', opacity: saving ? 0.7 : 1 }}>
...
function MemberRow({ member, onStatusChange }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 10, padding: 14, display: 'flex', gap: 12, alignItems: 'center' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 700, fontSize: 13, color: '#0F172A' }}>{member.customer_name}</div>
        <div style={{ fontSize: 11, color: '#64748B', marginTop: 2 }}>{member.customer_phone} · {member.plan_name}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
        ...
        {effectiveStatus === 'active' && (
          <button onClick={() => onStatusChange(member.id, 'cancelled')} style={{ fontSize: 10, background: 'none', border: 'none', color: '#EF4444', cursor: 'pointer', fontWeight: 600 }}>Cancel</button>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div>
          <h2 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#0F172A' }}>Memberships</h2>
          <p style={{ margin: 0, fontSize: 13, color: '#64748B' }}>Sell subscription plans with benefits & discounts</p>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'Active Members', value: activeMembers.length, color: '#10B981', bg: '#D1FAE5' },
          { label: 'Expiring in 7 days', value: expiringSoon.length, color: '#F59E0B', bg: '#FEF3C7' },
          { label: 'Plans Live', value: plans.filter(p => p.active).length, color: '#4F46E5', bg: '#EEF2FF' },
        ].map(s => (
      ...
      <div style={{ display: 'flex', gap: 4, background: '#F1F5F9', borderRadius: 10, padding: 4, marginBottom: 20 }}>
        {[{ id: 'members', label: '👤 Members' }, { id: 'plans', label: '🏆 Plans' }].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ flex: 1, padding: '8px 12px', borderRadius: 8, border: 'none', fontWeight: 700, fontSize: 13, cursor: 'pointer', background: tab === t.id ? '#fff' : 'transparent', color: tab === t.id ? '#4F46E5' : '#64748B', boxShadow: tab === t.id ? '0 1px 4px rgba(0,0,0,0.08)' : 'none' }}>{t.label}</button>
        ))}
      </div>

...
export default function ManageBooking() {
  ...
  if (loading) {
    return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94A3B8' }}>Loading your booking…</div>;
  }
  ...
  if (notFound) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, padding: 24 }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 700, color: '#0F172A' }}>Booking not found</div>
        <div style={{ fontSize: 13, color: '#64748B', textAlign: 'center' }}>This link may be incorrect or the booking may no longer exist.</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8FAFC', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '32px 16px' }}>
      <ToastContainer theme="light" position="top-center" />
      <div style={{ background: '#fff', borderRadius: 16, maxWidth: 420, width: '100%', padding: 24, boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
        {mode === 'view' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: 0.5 }}>Your Booking</div>
                <div style={{ fontSize: 18, fontWeight: 800, color: '#0F172A', marginTop: 2 }}>{appt.service_name}</div>
              </div>
            ...
            <div style={{ background: '#F8FAFC', borderRadius: 12, padding: 16, marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 700, color: '#10B981' }}>₹{Number(appt.service_price).toLocaleString('en-IN')}</div>
            </div>

            {canModify ? (
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => { setMode('reschedule'); setNewTime(''); }}
                  style={{ flex: 1, padding: 12, borderRadius: 10, border: '1px solid #E2E8F0', background: '#fff', color: '#0F172A', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                  📅 Reschedule
                </button>
                <button onClick={handleCancel}
                  style={{ flex: 1, padding: 12, borderRadius: 10, border: '1px solid #FCA5A5', background: '#FFF5F5', color: '#EF4444', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                  ✕ Cancel Booking
                </button>
              </div>
            ) : (
              <div style={{ textAlign: 'center', padding: 12, background: '#F8FAFC', borderRadius: 10, fontSize: 12, color: '#94A3B8' }}>
        ...
        {mode === 'reschedule' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: '#0F172A' }}>Reschedule</div>
              <button onClick={() => setMode('view')} style={{ background: 'none', border: 'none', color: '#94A3B8', cursor: 'pointer', fontSize: 13 }}>Cancel</button>
            </div>
            <div style={{ background: '#EEF2FF', borderRadius: 10, padding: '10px 14px', marginBottom: 16, fontSize: 13, fontWeight: 700, color: '#4F46E5' }}>
              {appt.service_name}
            </div>

            <div style={{ marginBottom: 18 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 10 }}>Select New Date</div>
              <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
                {getNext14Days().map(d => (
                  <button key={d.iso} onClick={() => { setNewDate(d.iso); setNewTime(''); }}
                    style={{ flexShrink: 0, width: 52, padding: '8px 4px', borderRadius: 10, border: '1px solid', cursor: 'pointer', textAlign: 'center', borderColor: newDate === d.iso ? '#4F46E5' : '#E2E8F0', background: newDate === d.iso ? '#4F46E5' : '#fff', color: newDate === d.iso ? '#fff' : '#475569' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              {appt.provider_id && dayAvailability.onTimeOff ? (
                ...
                <div style={{ textAlign: 'center', padding: 20, background: '#F8FAFC', borderRadius: 10, fontSize: 13, color: '#94A3B8' }}>Not available this day</div>
              ) : (
                    ...
                    return (
                      <button key={t} disabled={taken} onClick={() => !taken && setNewTime(t)}
                        style={{
                          ...
                          borderColor: taken ? '#F1F5F9' : (newTime === t ? '#4F46E5' : '#E2E8F0'),
                          background: taken ? '#F8FAFC' : (newTime === t ? '#4F46E5' : '#fff'),
                          color: taken ? '#CBD5E1' : (newTime === t ? '#fff' : '#475569'),
            ...
            <button onClick={handleReschedule} disabled={saving || !newTime}
              style={{ width: '100%', padding: 13, borderRadius: 10, border: 'none', background: (saving || !newTime) ? '#94A3B8' : '#4F46E5', color: '#fff', fontWeight: 800, fontSize: 14, cursor: (saving || !newTime) ? 'not-allowed' : 'pointer' }}>

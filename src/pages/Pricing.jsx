...
const DIST_PLANS = [
  { id: 'basic_distributor', name: 'Basic Distributor', price: priceOf('basic_distributor'), popular: false,
    features: [
      ...
      { label: 'Field sales & route beats', on: false },
  ...
  { id: 'pro_distributor', name: 'Pro Distributor', price: priceOf('pro_distributor'), popular: true,
    features: [
      ...
      { label: 'Field sales: routes, beats & visit tracking', on: true },
      { label: 'Auto route sequencing (cuts driving distance)', on: true },
      { label: 'Presale order booking from the field', on: true },
      { label: 'Live field team activity', on: true },
      { label: 'End-of-day cash & stock reconciliation', on: true },
      { label: 'Depot & warehouse stock tracking', on: true },
      { label: 'Up to 3 vehicles', on: true },
  ...
  { id: 'enterprise_distributor', name: 'Enterprise Distributor', price: priceOf('enterprise_distributor'), popular: false,
    features: [
      ...
      { label: 'Van sales — bill offline, sync later', on: true },
      { label: 'On-the-spot returns & credit notes', on: true },
      { label: 'Batch & expiry tracking (FEFO)', on: true },
      { label: 'Unlimited vehicles & field reps', on: true },
...
/* ─── Shared style tokens ───────────────────────────────────────────── */
const S = {
  check: { color: 'var(--c-success)', flexShrink: 0 },
  cross: { color: 'var(--c-ink-2)', flexShrink: 0 },
  th: { padding: '14px 16px', background: 'rgba(255,255,255,0.02)', color: 'var(--c-muted)', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'center' },
  td: { padding: '12px 16px', fontSize: '13px', borderBottom: '1px solid rgba(255,255,255,0.04)', textAlign: 'center', color: 'var(--c-faint)' },
  tdLabel: { padding: '12px 16px', fontSize: '13px', borderBottom: '1px solid rgba(255,255,255,0.04)', textAlign: 'left', color: 'var(--c-bg)', fontWeight: 500 },
};

function Cell({ val }) {
  if (val === true) return <Check size={16} style={S.check} />;
  if (val === false) return <X size={16} style={S.cross} />;
  return <span style={{ color: 'var(--c-bg)', fontWeight: 600, fontSize: '13px' }}>{val}</span>;
}

function PlanCard({ plan, idx, popular, onCta, cycle = 'monthly', pricing = null }) {
  const TIER_STYLES = [
    { border: 'rgba(100,116,139,0.5)', glow: '' },
    { border: 'var(--c-primary)', glow: '0 0 40px rgba(79,70,229,0.25)' },
  ...
  return (
    <div style={{
      ...
      {popular && (
        <div style={{ position: 'absolute', top: '-14px', left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(135deg,var(--c-primary-light),var(--c-primary))', color: 'var(--c-surface)', fontSize: '11px', fontWeight: 800, padding: '4px 14px', borderRadius: '99px', whiteSpace: 'nowrap', boxShadow: '0 0 20px rgba(79,70,229,0.5)', animation: 'pulse 2s ease-in-out infinite' }}>
          ⭐ Most Popular
        </div>
      )}
      <div style={{ marginBottom: '20px' }}>
        <div style={{ color: 'var(--c-faint)', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '8px' }}>{plan.name}</div>
        {(() => {
            ...
            return (
              <div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '22px', color: 'var(--c-muted)', textDecoration: 'line-through', fontWeight: 700 }}>₹{pr.base.toLocaleString('en-IN')}</span>
                  <span style={{ fontSize: '42px', fontWeight: 900, color: 'var(--c-bg)' }}>₹{pr.final.toLocaleString('en-IN')}</span>
                  <span style={{ color: 'var(--c-muted)', fontSize: '14px' }}>{cycleSuffix[cycle]}</span>
                </div>
                <div style={{ color: 'var(--c-success)', fontSize: '13px', fontWeight: 700, marginTop: 4 }}>Save {pr.totalPct}% vs monthly</div>
              </div>
            );
          }
          return (
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
              <span style={{ fontSize: '42px', fontWeight: 900, color: 'var(--c-bg)' }}>₹{(plan.price || 0).toLocaleString('en-IN')}</span>
              <span style={{ color: 'var(--c-muted)', fontSize: '14px' }}>{cycleSuffix.monthly}</span>
            </div>
          );
        })()}
        {plan.description && <p style={{ fontSize: '13px', color: 'var(--c-muted)', marginTop: '10px', lineHeight: 1.5 }}>{plan.description}</p>}
      </div>

      <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 28px', flex: 1, display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {features.map((f, i) => (
          <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', fontSize: '13px', color: f.on ? 'var(--c-bg)' : 'var(--c-ink-2)' }}>
      ...
      <Link to="/register" onClick={onCta}
        style={{ display: 'block', textAlign: 'center', padding: '13px', borderRadius: '10px', fontWeight: 700, fontSize: '14px', textDecoration: 'none',
          background: popular ? 'linear-gradient(135deg,var(--c-primary),var(--c-primary-light))' : 'rgba(255,255,255,0.06)',
          border: popular ? 'none' : `1px solid ${ts.border}`,
          color: 'var(--c-surface)', transition: 'opacity 0.2s',
        }}>
        Start Free Trial
      </Link>
    </div>
...
function FAQ({ q, a }) {
  ...
  return (
    <div style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      {open && <p style={{ margin: '0 0 18px', fontSize: '14px', color: 'var(--c-faint)', lineHeight: 1.7 }}>{a}</p>}
    </div>
  );
}
            ...
            {['shops', 'services', 'distributors'].map(m => (
              <button key={m} onClick={() => setMode(m)}
                style={{ padding: '9px 20px', borderRadius: '9px', border: 'none', cursor: 'pointer', fontSize: '13.5px', fontWeight: 700, fontFamily: "'Inter', system-ui, sans-serif", transition: 'all 0.2s',
                  background: mode === m ? 'linear-gradient(135deg,var(--c-primary-light),var(--c-primary))' : 'transparent',
                  color: mode === m ? 'var(--c-surface)' : T.textFaint,
            ...
            <div style={{ textAlign: 'center', marginBottom: 28 }}>
              <div style={{ display: 'inline-flex', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: 4, gap: 4, flexWrap: 'wrap' }}>
                {cycles.map(c => {
                  ...
                  return (
                    <button key={c} onClick={() => setCycle(c)} style={{ background: cycle === c ? 'linear-gradient(135deg,var(--c-primary-light),var(--c-primary))' : 'transparent', border: 'none', color: cycle === c ? 'var(--c-surface)' : 'var(--c-faint)', padding: '10px 22px', borderRadius: 9, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
                      {lbl[c]}{d > 0 && <span style={{ marginLeft: 6, fontSize: 11, color: cycle === c ? 'var(--c-surface)' : 'var(--c-success)', fontWeight: 800 }}>-{d}%</span>}
                    </button>
                  );
                })}
              </div>
              {offerOn && cycle !== 'monthly' && (
                <div style={{ marginTop: 12, display: 'inline-block', background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.35)', color: 'var(--c-success)', borderRadius: 20, padding: '6px 18px', fontSize: 13, fontWeight: 800 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 1, minWidth: 220 }}>
                ...
                <p style={{ margin: '6px 0 0', fontSize: 14, color: 'var(--c-faint)' }}>

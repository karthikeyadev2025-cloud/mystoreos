...
const STATUS_LABEL = {
  pending: { l: 'Pending', c: '#94A3B8' },
  counted: { l: 'Counted', c: '#CA8A04' },
  closed: { l: 'Closed', c: '#059669' },
  variance_flagged: { l: 'Variance flagged', c: '#DC2626' },
};
...
export default function FieldSettlementHistory() {
  ...
  const S = {
    card: { background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: 16, marginBottom: 10 },
  };
  ...
  return (
    <div style={{ padding: 20, maxWidth: 720, margin: '0 auto', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
      ...
      <h1 style={{ fontSize: 22, fontWeight: 900, color: '#0F172A', margin: '0 0 4px' }}>Day Reports</h1>
      <p style={{ fontSize: 13, color: '#64748B', margin: '0 0 16px' }}>
        Past van settlements — cash collected, UPI, and any variance flagged at close.
      </p>

      <select value={vehicleFilter} onChange={e => setVehicleFilter(e.target.value)}
        style={{ width: '100%', padding: '11px 13px', border: '1px solid #E2E8F0', borderRadius: 9, fontSize: 14, marginBottom: 16, background: '#fff' }}>
      ...
      {!loading && history.length > 0 && (
        <div style={{ ...S.card, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(110px,1fr))', gap: 10 }}>
          <div>
            <div style={{ fontSize: 10, color: '#64748B', fontWeight: 700, textTransform: 'uppercase' }}>Cash</div>
            <div style={{ fontSize: 16, fontWeight: 900, color: '#0F172A' }}>{inr(totals.cash)}</div>
          </div>
          <div>
            <div style={{ fontSize: 10, color: '#64748B', fontWeight: 700, textTransform: 'uppercase' }}>UPI</div>
            <div style={{ fontSize: 16, fontWeight: 900, color: '#0F172A' }}>{inr(totals.upi)}</div>
          </div>
          <div>
            <div style={{ fontSize: 10, color: '#64748B', fontWeight: 700, textTransform: 'uppercase' }}>On Credit</div>
            <div style={{ fontSize: 16, fontWeight: 900, color: '#0F172A' }}>{inr(totals.credit)}</div>
          </div>
          <div>
            <div style={{ fontSize: 10, color: '#64748B', fontWeight: 700, textTransform: 'uppercase' }}>Variance Days</div>
            <div style={{ fontSize: 16, fontWeight: 900, color: totals.varianceDays > 0 ? '#DC2626' : '#059669' }}>{totals.varianceDays}</div>
          </div>
        </div>
      )}
          ...
          <div key={h.id} style={S.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 14, fontWeight: 800, color: '#0F172A' }}>{h.vehicleCode}</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, fontSize: 12 }}>
              <div>
                <div style={{ color: '#94A3B8', fontSize: 10 }}>Cash</div>
                <div style={{ fontWeight: 700 }}>{inr(h.countedCash ?? h.expectedCash)}</div>
              </div>
              <div>
                <div style={{ color: '#94A3B8', fontSize: 10 }}>UPI</div>
                <div style={{ fontWeight: 700 }}>{inr(h.countedUpi ?? h.expectedUpi)}</div>
              </div>
              <div>
                <div style={{ color: '#94A3B8', fontSize: 10 }}>On Credit</div>
                <div style={{ fontWeight: 700 }}>{inr(h.expectedCredit)}</div>
              </div>
            </div>

            {h.status === 'variance_flagged' && (
              <div style={{ marginTop: 10, padding: '8px 12px', background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, fontSize: 11, color: '#991B1B', display: 'flex', gap: 6 }}>

...
export default function DirectSale() {
  ...
  if (loading) {
    return <div style={{ padding: 60, textAlign: 'center', color: 'var(--c-muted)', fontSize: '16px', fontWeight: 'bold' }}>Loading Wholesale Inventory &amp; Linked Shops…</div>;
  }
    ...
    <div style={{ padding: '24px 16px', maxWidth: 960, margin: '0 auto', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", color: 'var(--c-ink)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <button onClick={() => navigate('/distributor')}
          style={{ background: 'var(--c-line-soft)', border: '1px solid var(--c-line)', color: 'var(--c-primary)', padding: '8px 16px', borderRadius: 10, fontSize: 13, fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button 
            ...
            style={{ ...S.badgePill, background: isInterstate ? 'var(--c-warning-soft)' : 'var(--c-line-soft)', color: isInterstate ? 'var(--c-warning-strong)' : 'var(--c-ink-2)', border: '1px solid var(--c-line-strong)', cursor: 'pointer' }}
          >
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 24, fontWeight: 900, color: 'var(--c-ink)', margin: 0 }}>Wholesale Billing &amp; Sales Terminal</h1>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button onClick={() => handlePrintInvoice('a4', true)}
                style={{ background: 'var(--c-ink)', color: 'var(--c-surface)', border: 'none', padding: '10px 14px', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {lastSale.phone && (
                <button onClick={handleShareWhatsApp}
                  style={{ background: 'var(--c-success-strong)', color: 'var(--c-surface)', border: 'none', padding: '10px 14px', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={S.card}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              ...
              <button 
                ...
                style={{ background: 'var(--c-primary-soft)', color: 'var(--c-primary)', border: '1px solid var(--c-primary-border)', borderRadius: 6, padding: '2px 8px', fontSize: 11, fontWeight: 800, cursor: 'pointer' }}
              >
                + Add New Customer
              </button>
            </div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {shopSearch && (
                <button 
                  ...
                  style={{ position: 'absolute', right: 10, top: 10, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--c-faint)' }}
                >
                  <X size={14} />
                </button>
              )}
        ...
        {selectedShopObj && (
          <div style={{ background: 'var(--c-bg)', border: '1px solid var(--c-line)', borderRadius: 12, padding: 12, marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
            <div>
              <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--c-ink-2)' }}>PARTY CREDIT LEDGER STATUS</span>
              <div style={{ fontSize: 14, fontWeight: 900, color: selectedShopObj.owed > 0 ? 'var(--c-danger-strong)' : 'var(--c-success-strong)', marginTop: 2 }}>
            ...
            {selectedShopObj.phone && (
              <span style={{ fontSize: 12, color: 'var(--c-muted)', fontWeight: 600 }}>📞 {selectedShopObj.phone}</span>
            )}
          </div>
        )}
      ...
      <div style={S.card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
          ...
          {products.length > 0 && (
            <div style={{ position: 'relative', width: isMobile ? '100%' : 300 }}>
              <Search size={14} style={{ position: 'absolute', left: 10, top: 12, color: 'var(--c-primary)' }} />
              <input 
                type="text" 
                value={searchQuery} 
                onChange={e => setSearchQuery(e.target.value)} 
                placeholder="⚡ Fast search name, SKU, HSN, barcode..." 
                style={{ ...S.input, paddingLeft: 30, paddingRight: searchQuery ? 30 : 10, padding: '7px 30px 7px 30px', fontSize: 12, borderColor: 'var(--c-primary-light)' }} 
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery('')} style={{ position: 'absolute', right: 8, top: 8, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--c-faint)' }}>
                  <X size={14} />
                </button>
              )}
            </div>
          ...
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 10, marginBottom: 10, scrollbarWidth: 'none' }}>
            {filteredProducts.slice(0, 10).map(p => (
              <button 
                ...
                style={{ 
                  background: selectedProdId === p.id ? 'var(--c-primary)' : 'var(--c-line-soft)',
                  color: selectedProdId === p.id ? 'var(--c-surface)' : 'var(--c-ink-2)',
                  border: `1px solid ${selectedProdId === p.id ? 'var(--c-primary)' : 'var(--c-line-strong)'}`,
                  borderRadius: 8,
                  padding: '5px 10px',
                  fontSize: 11,
                  fontWeight: 700,
        ...
        {isMobile ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--c-muted)' }}>Select Product</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--c-muted)' }}>Rate (₹/unit)</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--c-muted)' }}>Discount %</label>
            ...
            <button onClick={addLineItem}
              style={{ width: '100%', background: 'linear-gradient(135deg, var(--c-primary), var(--c-primary-hover))', color: 'var(--c-surface)', border: 'none', padding: '14px', borderRadius: 10, fontWeight: 800, fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, boxShadow: '0 4px 12px rgba(79,70,229,0.3)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 1fr 1fr 1fr auto', gap: 10, alignItems: 'end' }}>
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--c-muted)' }}>
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--c-muted)' }}>Loose Jars / Units</label>
              <input 
                type="number" 
                inputMode="numeric" 
            ...
            <button onClick={addLineItem}
              style={{ background: 'linear-gradient(135deg, var(--c-primary), var(--c-primary-hover))', color: 'var(--c-surface)', border: 'none', padding: '11px 18px', borderRadius: 10, fontWeight: 800, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, boxShadow: '0 4px 12px rgba(79,70,229,0.3)' }}>
      ...
      {cart.length > 0 && (
        <div style={S.card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <h3 style={{ margin: 0, fontSize: 16, fontWeight: 900, color: 'var(--c-ink)' }}>
              Invoice Line Items ({cart.length})
            </h3>
            <span style={{ fontSize: 12, color: 'var(--c-muted)', fontWeight: 700 }}>
          ...
          {isMobile ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {cart.map((item) => (
                <div key={item.productId} style={{ background: 'var(--c-bg)', border: '1px solid var(--c-line)', borderRadius: 12, padding: 14 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 14, color: 'var(--c-ink)' }}>{item.name}</div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--c-surface)', border: '1px solid var(--c-line-strong)', borderRadius: 8, padding: '8px 10px', marginTop: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <button onClick={() => updateLineQty(item.productId, -1)} style={{ background: 'var(--c-line-soft)', border: '1px solid var(--c-line-strong)', borderRadius: 4, width: 26, height: 26, fontWeight: 800, cursor: 'pointer' }}>-</button>
                      <span style={{ fontWeight: 800, fontSize: 14 }}>{item.qty} units</span>
                      <button onClick={() => updateLineQty(item.productId, 1)} style={{ background: 'var(--c-line-soft)', border: '1px solid var(--c-line-strong)', borderRadius: 4, width: 26, height: 26, fontWeight: 800, cursor: 'pointer' }}>+</button>
                    </div>

                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 11, color: 'var(--c-muted)' }}>₹{item.rate} / unit</div>
                      <div style={{ fontWeight: 900, fontSize: 15, color: 'var(--c-ink)' }}>₹{item.lineTotal.toFixed(0)}</div>
                    </div>
                  </div>
                </div>
              ))}
                  ...
                  {cart.map((item, idx) => (
                    <tr key={item.productId} style={{ borderBottom: '1px solid var(--c-line-soft)' }}>
                      ...
                      <td style={{ padding: '12px 8px', textAlign: 'center' }}>
                        {item.boxes > 0 && <span style={{ background: 'var(--c-primary-soft)', color: 'var(--c-primary-hover)', padding: '2px 6px', borderRadius: 6, fontSize: 11, fontWeight: 800, marginRight: 4 }}>{item.boxes} Box{item.boxes > 1 ? 'es' : ''}</span>}
                        {item.looseUnits > 0 && <span style={{ background: 'var(--c-warning-soft)', color: 'var(--c-warning-strong)', padding: '2px 6px', borderRadius: 6, fontSize: 11, fontWeight: 800 }}>{item.looseUnits} Loose</span>}
                        {item.boxes === 0 && item.looseUnits === 0 && '—'}
                      </td>
                      <td style={{ padding: '12px 8px', textAlign: 'center' }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                          <button onClick={() => updateLineQty(item.productId, -1)} style={{ background: 'var(--c-line-soft)', border: '1px solid var(--c-line-strong)', borderRadius: 4, width: 22, height: 22, fontWeight: 800, cursor: 'pointer' }}>-</button>
                      ...
                      <td style={{ padding: '12px 8px', textAlign: 'right', color: item.discPct > 0 ? 'var(--c-danger-strong)' : 'var(--c-muted)', fontWeight: 700 }}>{item.discPct > 0 ? `${item.discPct}%` : '—'}</td>
          ...
          <div style={{ marginTop: 20, paddingTop: 16, borderTop: '2px solid var(--c-line-soft)' }}>
            <h4 style={{ margin: '0 0 12px', fontSize: 14, fontWeight: 800, color: 'var(--c-ink-2)' }}>🚚 Delivery Logistics &amp; Transport Details</h4>
          ...
          <div style={{ marginTop: 20, background: 'var(--c-bg)', border: '1px solid var(--c-line)', borderRadius: 14, padding: 18, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <div>
            ...
            <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 13, color: 'var(--c-ink-2)' }}>
                ...
                {totals.flatDiscount > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--c-danger-strong)' }}>
                ...
                <div style={{ borderTop: '1px solid var(--c-line-strong)', paddingTop: 8, marginTop: 4, display: 'flex', justifyContent: 'space-between', fontSize: 22, fontWeight: 900, color: 'var(--c-success-strong)' }}>
                ...
                return (
                  <button key={m.id} disabled={blocked} onClick={() => !blocked && setPayMode(m.id)}
                    style={{
                      ...
                      border: `2px solid ${payMode === m.id ? 'var(--c-primary)' : 'var(--c-line)'}`,
                      background: payMode === m.id ? 'var(--c-primary-soft)' : 'var(--c-surface)',
                      color: payMode === m.id ? 'var(--c-primary-hover)' : 'var(--c-ink-2)',
      ...
      {showAddCustModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16 }}>
          <div style={{ background: 'var(--c-surface)', borderRadius: 16, padding: 24, maxWidth: 480, width: '100%', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 900, color: 'var(--c-ink)' }}>+ Add New Retail Shop / Party</h3>
              <button onClick={() => setShowAddCustModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--c-muted)' }}>
                <X size={20} />
              </button>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
              <button onClick={() => setShowAddCustModal(false)} style={{ background: 'var(--c-line-soft)', border: '1px solid var(--c-line-strong)', color: 'var(--c-ink-2)', padding: '10px 16px', borderRadius: 10, fontWeight: 700, cursor: 'pointer' }}>
                Cancel
              </button>
              <button onClick={handleAddQuickCustomer} style={{ background: 'var(--c-primary)', border: 'none', color: 'var(--c-surface)', padding: '10px 20px', borderRadius: 10, fontWeight: 800, cursor: 'pointer' }}>
                Save &amp; Select
              </button>
            </div>
          </div>

  ...
  return (
    <div className="enterprise-wrapper" style={{ minHeight: '100vh', background: '#F4F5F7', color: '#0F172A', padding: '20px', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      {!selectedShop ? (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  {shop.logo ? (
                    ...
                    <div style={{ width: '44px', height: '44px', borderRadius: '8px', background: '#F8FAFC', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #E2E8F0' }}>
                      <Store size={22} color="#64748B" />
                    </div>
                  )}
                  <div>
                    <h3 style={{ margin: 0, fontSize: '14px', fontWeight: '700', color: '#0F172A' }}>{shop.name}</h3>
                    <p style={{ margin: 0, fontSize: '11px', color: '#64748B' }}>{shop.phone}</p>
                  </div>
                </div>
                {shop.gstin && (
                  <div style={{ background: '#F8FAFC', padding: '6px 10px', borderRadius: '6px', fontSize: '11px', color: '#475569', border: '1px solid #E2E8F0' }}>
                    <span style={{ color: '#059669', fontWeight: 'bold' }}>GSTIN: </span>{shop.gstin}
                  </div>
                )}
              </div>
            ))}
        ...
        <div>
          <button onClick={() => setSelectedShop(null)} style={{ background: 'transparent', border: 'none', color: '#475569', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', padding: '0 0 16px 0', fontSize: '13px', fontWeight: 'bold' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'flex-end' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <select
                    ...
                    style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', color: '#0F172A', padding: '6px 10px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}
                  >
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '20px' }}>
            {statCard('Invoices', gstSummary.invoiceCount, '#B45309')}
            {statCard('Total Revenue', `₹${gstSummary.totalRevenue.toLocaleString('en-IN', { maximumFractionDigits: 0 })}`, '#059669')}
            {statCard('Taxable Value', `₹${gstSummary.totalTaxable.toLocaleString('en-IN', { maximumFractionDigits: 0 })}`, '#0F172A')}
            {statCard('CGST', `₹${gstSummary.totalCGST.toFixed(0)}`, '#D97706')}
            {statCard('SGST', `₹${gstSummary.totalSGST.toFixed(0)}`, '#D97706')}
            {statCard('IGST', `₹${gstSummary.totalIGST.toFixed(0)}`, '#DC2626')}
          </div>
          ...
          <div style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '14px', padding: '20px', boxShadow: '0 1px 2px rgba(15,23,42,0.06)' }}>
            ...
            {loading ? (
              ...
              <p style={{ color: '#64748B', fontSize: '13px' }}>No completed sales in this period.</p>
            ) : (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                  <thead>
                    <tr style={{ background: '#F8FAFC', color: '#475569', textAlign: 'left' }}>
                      ...
                      <th style={{ padding: '10px 12px', borderBottom: '1px solid #E2E8F0' }}>Invoice #</th>
                      <th style={{ padding: '10px 12px', borderBottom: '1px solid #E2E8F0' }}>Customer / Party</th>
                      <th style={{ padding: '10px 12px', borderBottom: '1px solid #E2E8F0' }}>GST</th>
                      <th style={{ padding: '10px 12px', borderBottom: '1px solid #E2E8F0', textAlign: 'right' }}>Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredOrders.map(order => {
                      ...
                      return (
                        <tr key={order.id} style={{ borderBottom: '1px solid #F1F5F9' }}>
                          <td style={{ padding: '10px 12px', color: '#0F172A' }}>{new Date(order.date || order.createdAt).toLocaleDateString('en-IN')}</td>
                          <td style={{ padding: '10px 12px', color: '#475569', fontFamily: 'monospace', fontSize: '11px' }}>#{order.id.slice(0, 8).toUpperCase()}</td>
                          <td style={{ padding: '10px 12px', color: '#0F172A' }}>
                            {customerName}
                            {order.customerGstin && <span style={{ display: 'block', fontSize: '10px', color: '#059669' }}>{order.customerGstin}</span>}
                          </td>
                          <td style={{ padding: '10px 12px', color: '#B45309', fontSize: '11px' }}>
                            {gstRate > 0 ? `${gstRate}% · ₹${gstAmt.toFixed(0)}` : '—'}
                          </td>
                          <td style={{ padding: '10px 12px', color: '#059669', fontWeight: 'bold', textAlign: 'right' }}>₹{total.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                  <tfoot>
                    <tr style={{ background: '#F8FAFC' }}>
                      <td colSpan={4} style={{ padding: '10px 12px', fontWeight: 'bold', color: '#0F172A' }}>Period Total</td>
                      <td style={{ padding: '10px 12px', color: '#059669', fontWeight: '800', textAlign: 'right', fontSize: '14px' }}>

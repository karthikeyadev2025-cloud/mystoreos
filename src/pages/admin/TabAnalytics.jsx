...
import { RefreshCw, TrendingDown, TrendingUp, Target, Brain } from 'lucide-react';
...
export default function TabAnalytics() {
  ...
  if (loading) return <div style={{ textAlign: 'center', color: 'var(--c-muted)', padding: '60px' }}>Loading analytics...</div>;
  ...
  return (
    <div className="admin-tab-content">
      ...
      <div style={S.card}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={growth} margin={{ top: 5, right: 10, bottom: 0, left: -10 }}>
            ...
            <Area type="monotone" dataKey="shops" name="Shops" stroke="#12457A" fill="url(#aShop)" strokeWidth={2} dot={false} />
            <Area type="monotone" dataKey="customers" name="Customers" stroke="#4A7CAD" fill="url(#aCust)" strokeWidth={2} dot={false} />
            <Area type="monotone" dataKey="distributors" name="Distributors" stroke="#22C55E" strokeWidth={2} dot={false} fill="transparent" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div style={S.card}>
        ...
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={revenue} margin={{ top: 5, right: 10, bottom: 0, left: -10 }}>
            ...
            <Bar dataKey="shops" name="Shops" fill="#12457A" stackId="a" radius={[0, 0, 0, 0]} />
            <Bar dataKey="distributors" name="Distributors" fill="#4A7CAD" stackId="a" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="admin-grid-2col">
        ...
        <div style={S.card}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
            <Target size={15} color="var(--c-primary-light)" />
      ...
      <div className="admin-grid-2col">
        <div style={S.card}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
            <TrendingDown size={15} color="var(--c-danger)" />
          ...
          {expiredTrials.length > 5 && <div style={{ color: 'var(--c-muted)', fontSize: '12px', marginTop: '8px' }}>+{expiredTrials.length - 5} more in Exports tab</div>}
        </div>
        ...
        <div style={S.card}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
              ...
              <tbody>
                {cohort.slice(-4).map((row) => (
                  <tr key={row.month}>
                    <td style={{ ...S.td, padding: '6px 8px', color: 'var(--c-muted)', fontSize: '11px' }}>{row.month}</td>
                    {[100, Math.max(0, row.retention), Math.max(0, row.retention - 15)].map((val, _j) => {
                      const idx = Math.min(5, Math.floor(val / 20));
                      return <td key={_j} style={{ ...S.td, padding: '6px 8px', background: HEAT_COLORS[idx], color: val > 40 ? 'var(--c-ink)' : 'var(--c-ink-2)', textAlign: 'center', fontWeight: 600, fontSize: '11px' }}>{val}%</td>;
                    })}
                  </tr>
                ))}
              </tbody>
          ...
          <div style={{ color: 'var(--c-muted)', fontSize: '11px', marginTop: '8px' }}>*Estimated — connect real order data for precise cohort analysis</div>
        </div>
      </div>
      <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>

      {/* ── AI Demand Forecasting — Platform Overview ── */}
      <div style={{ background: 'var(--c-primary-soft)', border: '1px solid var(--c-primary-border)', borderRadius: '12px', padding: '20px', marginTop: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
          <Brain size={20} color="var(--c-primary)" />
          <div>
            <div style={{ color: 'var(--c-ink)', fontWeight: 700, fontSize: '15px' }}>AI Demand Forecasting — Platform Feature</div>
            <div style={{ color: 'var(--c-muted)', fontSize: '12px' }}>Available on Pro + Enterprise shop plans. Runs client-side — no API cost.</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {[
            { label: 'Trial / Starter', badge: 'Locked', color: 'var(--c-muted)' },

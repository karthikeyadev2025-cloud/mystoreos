...
const S = {
  ...
  badge: (role) => ({ background: `${ROLE_COLORS[role] || '#64748B'}15`, color: ROLE_COLORS[role] || '#64748B', padding: '2px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 600, textTransform: 'capitalize' }),
  btn: (color = '#4F46E5') => ({ background: `${color}15`, border: `1px solid ${color}30`, color, borderRadius: '8px', padding: '6px 12px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif', display: 'flex', alignItems: 'center', gap: '4px' }),
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <form onSubmit={submit} style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '14px', padding: '24px', width: '340px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
        ...
        <p style={{ color: '#64748B', fontSize: '13px', marginBottom: '14px' }}>Set new password for <b>{user.name}</b></p>
    ...
    <div className="admin-tab-content">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ color: '#64748B', fontSize: '13px', marginTop: '4px' }}>{users.length} total users across all roles</p>
        </div>
      ...
      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '20px' }}>
        {ROLES.map(r => (
          <button key={r} onClick={() => { setFilterRole(r); setPage(0); }} style={{ padding: '6px 12px', borderRadius: '20px', border: `1px solid ${filterRole === r ? (ROLE_COLORS[r] || '#4F46E5') : '#E5E7EB'}`, background: filterRole === r ? `${ROLE_COLORS[r] || '#4F46E5'}15` : '#FFFFFF', color: filterRole === r ? (ROLE_COLORS[r] || '#4F46E5') : '#475569', fontSize: '12px', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: filterRole === r ? 600 : 400, textTransform: 'capitalize', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                  ...
                  <tr key={u.id}>
                    <td style={S.td}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        {u.avatar || u.logo
                          ...
                          : <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: `${ROLE_COLORS[u.role] || '#64748B'}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 700, color: ROLE_COLORS[u.role] || '#4F46E5', flexShrink: 0 }}>{(u.name||'U')[0]}</div>
                        }
                    ...
                    <td style={S.td}>
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'nowrap' }}>
                        ...
                        <button disabled={isBusy} onClick={() => setResetModal(u)} style={S.btn('#475569')}><Key size={12} />Reset PW</button>
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #E5E7EB' }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} style={{ ...S.btn('#475569'), padding: '6px 12px' }}>Previous</button>
              <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} style={{ ...S.btn('#475569'), padding: '6px 12px' }}>Next</button>
            </div>
          </div>
        )}
      </div>
